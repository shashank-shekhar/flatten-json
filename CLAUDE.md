...
- `src/extension.ts` — VS Code glue. `activate()` registers the `flatten-json.flatten` command. The command flow: pick a JSON source (Active Document, Active Selection, Clipboard, or Pick a File...) → read the raw text → **if the source is Active Selection**, pick (`getSelectionMode`) as-is vs. absolute-path flattening; absolute path derives a key prefix via `getSelectionPrefix`, which uses `jsonc-parser`'s `getLocation` on the whole document at the selection's start offset (dropping the last segment when `isAtPropertyKey`) → pick a separator (`getSeparator`) → `parseJson` + `flattenJson(parsed, separator, prefix)` → open the result as a new untitled `json` document. All three pickers go through the `pickRemembered` helper: it persists the last choice in `context.globalState` (`flatten-json.lastSource` / `lastSelectionMode` / `lastSeparator`) and **pre-highlights it as the active item so pressing Enter repeats the previous run, without reordering the list**. It uses `createQuickPick` rather than `showQuickPick` because the latter can't set the initially-active item. Keep source-reading (`getSourceText`) and flattening (`flatten.ts`) separate; new JSON sources should be added as new cases in `getSourceText`'s switch (and `SOURCE_OPTIONS`), not by touching `flatten.ts`.
